...
**Created:** 2026-03-03
**Last Updated:** 2026-03-12 (Session 32: B9 FIXED for real — os._exit(0) in SIGINT handler)
...
| 8.8 | Populate project summaries for active projects | [x] | 2026-03-11 | mb (active), jg (active), gen (active), js (paused). Full format per ARCHITECTURE_MERGE_PLAN.md. |
| 8.9 | Update 6 doc files + end-session protocol | [x] | 2026-03-11 | Done in session 25. 6 files updated. |
| 8.10 | Final test, commit, merge to main, tag | [ ] | | Tag: post-architecture-merge |
| 8.11 | Rewrite 111 old notes with Opus 4.6 | [>] | | 15/113 done (sessions 1-15). 98 remaining. Sessions 1-10 rewritten in session 29, sessions 11-15 in session 31. |
...
| B8 | "1 MCP server failed" on /exit | LIKELY FIXED | 28 | Only brain-server running (/mcp shows 1 server). Gmail/Calendar gone. Validate with debug logs next session. |
| B9 | "1 MCP server failed" — brain-server SIGINT crash at /exit | FIXED | 32 | Root cause: brain-server SIGINT handler used sys.exit(0) which raises SystemExit — caught by FastMCP asyncio loop, process doesn't die, CC escalates to SIGTERM and counts failure. Fix: os._exit(0) for hard immediate exit. Commit ed2d7c8. Prior fix (9d84a2a, session 31) was insufficient. |
...
| # | Step | Status | Notes |
|---|------|--------|-------|
| 9.1 | Deep code review — all scripts, hooks, MCP server | [ ] | Line-by-line audit. No dead code, no stale references, no security issues. |
| 9.2 | Fix #3: Z suffix timestamps (open from session 19) | [ ] | Minor but must be clean for public. |
| 9.3 | Run brain_health.py — must be 9/9 PASS | [ ] | Final health check. |
| 9.4 | Run clean_transcripts.py — clean up any new typos | [ ] | |
| 9.5 | Security audit — scan all files for personal data, API keys, paths | [ ] | Re-run the audit from Step 7.6. |

### Documentation
| # | Step | Status | Notes |
|---|------|--------|-------|
| 9.6 | README.md — complete rewrite for public audience | [ ] | Must be clear, professional, compelling. First impression for stars. |
| 9.7 | CLAUDE_BRAIN_HOW_TO.md — review for completeness | [ ] | User guide must be accurate, step-by-step, no dead references. |
| 9.8 | config.yaml.example — verify all options documented | [ ] | |
| 9.9 | CHANGELOG.md — create or update with all features | [ ] | |
| 9.10 | LICENSE — verify MIT, no personal info beyond what's intended | [ ] | Decision 95 context. |

### Beta Testing
| # | Step | Status | Notes |
|---|------|--------|-------|
| 9.11 | Friend (Mac) clones repo, runs brain-setup.py | [ ] | Phase 7.8 — the original beta test step. |
| 9.12 | Friend runs through HOW_TO from scratch | [ ] | Fresh eyes. What's confusing? What breaks? |
| 9.13 | Fix all issues found in beta test | [ ] | |

### Marketing & Launch Strategy
| # | Step | Status | Notes |
|---|------|--------|-------|
| 9.14 | Identity decision: use real name (Mike Dolan) or handle? | [ ] | Revisit Decision 95. Professional considerations. |
| 9.15 | Social media setup: determine which platforms needed | [ ] | Twitter/X? Reddit? LinkedIn? What's the Claude community on? |
| 9.16 | Research: find ALL YouTube videos about Claude Code + memory | [ ] | Contact creators, offer early access / demo. |
| 9.17 | Research: find ALL relevant communities (Reddit, Discord, forums) | [ ] | r/ClaudeAI, Anthropic Discord, Claude Code GitHub Discussions, etc. |
| 9.18 | Research: how to approach Hacker News | [ ] | HN launch strategy: timing, title, Show HN format, comment engagement. |
| 9.19 | Research: how to contact Boris (Claude Code creator) | [ ] | Direct outreach — GitHub, Twitter, Anthropic channels. |
| 9.20 | Prepare launch post / announcement text | [ ] | Short, clear, focused on the problem it solves. No hype. |
| 9.21 | Create demo video or GIF showing brain in action | [ ] | Visual proof. Shows setup → session → memory in action. |
| 9.22 | Switch repo from PRIVATE to PUBLIC | [ ] | THE moment. Only after everything above is done. |
| 9.23 | Post to communities (HN, Reddit, YouTube creators, Boris) | [ ] | Coordinated — don't spray everywhere at once. |
| 9.24 | Monitor issues/discussions for first 48 hours | [ ] | First impressions matter. Respond fast. |
