...
def build_email_html(conn, days, stats, prev_stats, summaries, decisions,
    ...
    content += 'This report is designed to be forwarded to stakeholders.<br>'
    content += 'To change frequency: edit crontab -e</div>'
...
def build_daily_html(conn, stats, summaries, decisions, labels, since):
    ...
    html += f'claude-brain &middot; {now.strftime("%Y-%m-%d %H:%M")} &middot; Local data only<br>'
    html += 'To change frequency: edit crontab -e</div>'
...
def main():
    ...
    try:
        ...
        if args.project:
            ...
            subject = f"[Weekly] {period_start.strftime('%b %d')}-{now.strftime('%b %d')}: {total_s} sessions, {active_p} projects"
